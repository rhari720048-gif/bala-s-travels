...
import { 
  ShieldCheck, Lock, Mail, Key, LogOut, Search, Filter, 
  Trash2, Phone, MessageCircle, RefreshCw, CheckCircle2, Clock, 
  Calendar, MapPin, Car, ArrowLeft, ChevronDown
} from 'lucide-react';
import { 
  getEnquiries, updateEnquiryStatus, deleteEnquiry, clearAllEnquiries 
      ...
      <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center p-4 relative overflow-hidden">
        ...
        <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6 relative z-10">
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-xs text-slate-400">Secure Dispatch & Enquiry Management Desk</p>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-2xs space-y-1">
            <span className="text-xs text-slate-500 font-bold uppercase tracking-wider block">Total Enquiries</span>
            <span className="text-2xl sm:text-3xl font-black text-slate-900">{totalCount}</span>
          </div>

          <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-2xs space-y-1">
            <span className="text-xs text-brand-red font-bold uppercase tracking-wider block">New Unread</span>
            <span className="text-2xl sm:text-3xl font-black text-brand-red">{newCount}</span>
          </div>

          <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-2xs space-y-1">
            <span className="text-xs text-amber-600 font-bold uppercase tracking-wider block">In Contact</span>
            <span className="text-2xl sm:text-3xl font-black text-amber-600">{contactedCount}</span>
          </div>

          <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-2xs space-y-1">
            <span className="text-xs text-emerald-600 font-bold uppercase tracking-wider block">Confirmed Rides</span>
            <span className="text-2xl sm:text-3xl font-black text-emerald-600">{confirmedCount}</span>
          </div>
        ...
        <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-2xs flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="relative w-full sm:max-w-xs">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 bg-slate-50 text-slate-900 rounded-xl border border-slate-200 text-xs font-semibold focus:outline-none focus:border-brand-red"
            />
          </div>

          {/* STATUS FILTER TABS */}
          <div className="flex items-center gap-1.5 overflow-x-auto w-full sm:w-auto">
            {['All', 'New', 'Contacted', 'Confirmed'].map((status) => (
              <button
                key={status}
            ...
            <button
              ...
              className="p-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 transition-colors ml-2"
              title="Refresh Data"
            >
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            ...
            <span className="text-xs text-slate-500 font-medium">Real-time Form Submissions</span>
          </div>
              ...
              <table className="w-full text-left text-xs">
                ...
                <thead className="bg-slate-50 border-b border-slate-200 text-slate-600 font-extrabold uppercase tracking-wider text-[10px]">
                  <tr>
                    ...
                    <th className="px-6 py-3.5">Vehicle Preference</th>
                  ...
                  {filteredEnquiries.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <div className="text-slate-500 font-semibold flex items-center gap-1 mt-0.5">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <select
                          ...
                          className={`px-2.5 py-1 rounded-full text-[11px] font-extrabold border cursor-pointer appearance-none ${
                            item.status === 'New'
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right space-x-2">
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-emerald-50 hover:bg-brand-green text-brand-green hover:text-white font-extrabold text-[11px] transition-colors"
                        ...
                        <a
                          ...
                          className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-slate-800 text-slate-700 hover:text-white font-extrabold text-[11px] transition-colors"
                          title="Call Customer"
                        >
